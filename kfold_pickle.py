...
correlation = pd.concat([x,train['Loan_Status']], axis=1).corr()
...
x['TotalIncome_l'] = np.log(x['TotalIncome'])
test['TotalIncome_l'] = np.log(test['TotalIncome'])
...
x['Balance'] = x['TotalIncome'] - (x['EMI']*1000)
test['Balance'] = test['TotalIncome'] - (test['EMI']*1000)

sns.distplot(x.Balance)

x.drop(['ApplicantIncome', 'CoapplicantIncome', 'LoanAmount'
        , 'Loan_Amount_Term'], axis=1, inplace=True)
test.drop(['ApplicantIncome', 'CoapplicantIncome', 'LoanAmount'
        , 'Loan_Amount_Term'], axis=1, inplace=True)

latest_x = x.drop(['Dependents_0', 'Self_Employed_Yes', 
                   'TotalIncome_l', 'Balance', 'EMI', 'Gender_Male',
                   'TotalIncome', 'Dependents_1', 'Dependents_2',
                   'Property_Area_Urban', 'Education_Not Graduate',
                   'Married_Yes', 'Property_Area_Semiurban'], axis=1)

latest_test = test.drop(['Dependents_0', 'Self_Employed_Yes', 
                   'TotalIncome_l', 'Balance', 'EMI', 'Gender_Male',
                   'TotalIncome', 'Dependents_1', 'Dependents_2',
                   'Property_Area_Urban', 'Education_Not Graduate',
                   'Married_Yes', 'Property_Area_Semiurban'], axis=1)

correlation = pd.concat([train['Loan_Status'], latest_x], axis=1).corr()

from sklearn.model_selection import train_test_split


from sklearn.linear_model import LogisticRegression
regressor = LogisticRegression(C=5.0, random_state=1)
...
x_train, x_test, y_train, y_test = train_test_split(latest_x, y, test_size=0.25,
                                                        random_state=0)
...
important = pd.Series(regressor.feature_importances_, index=x.columns)

accuracy_score(y_test, pickled.predict(x_test))
model = pickled

prediction = model.predict(latest_test)
...
pd.DataFrame(submission).to_csv('Naive1.csv', index=False)
